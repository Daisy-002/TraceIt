...
const Dashboard: React.FC = () => {
  ...
  useEffect(() => {
    const fetchItems = async () => {
      try {
        setError('');
        // Query to get most recent 50 items sorted by creation date (optimized for speed)
      ...
      <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: '100vh' }}>
        <div style={{ textAlign: 'center' }}>
          ...
          <button 
            ...
            style={{ ...S.btnPrimary, cursor: 'pointer' } as React.CSSProperties}
          >
            Try Again
          </button>
        </div>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', minHeight: '100vh', background: '#0f0f0f', color: '#f0ede8', fontFamily: "'DM Sans', 'Segoe UI', sans-serif" } as React.CSSProperties}>
      ...
      <div style={{ flex: 1, maxWidth: 1200, margin: '0 auto', width: '100%', padding: '32px 16px' }}>
        ...
        <div style={{ marginBottom: 40 }}>
          <h1 style={{ fontSize: 28, fontWeight: 700, letterSpacing: '-0.6px', margin: '0 0 6px', color: '#f0ede8' }}>Browse Lost & Found Items</h1>
          <p style={{ fontSize: 14, color: '#666', margin: 0 }}>Find your missing items or help others reunite with theirs</p>
        </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 16 }}>
            <div style={{ gridColumn: 'span 2' }}>
              ...
              <input
                ...
                style={{ width: '100%', background: '#0f0f0f', border: '1px solid #222', color: '#f0ede8', padding: '10px 12px', borderRadius: 8, fontSize: 14, outline: 'none' }}
              />
            </div>
            <div>
              ...
              <select
                value={filter}
                onChange={(e) => setFilter(e.target.value as 'all' | 'found' | 'lost')}
        ...
        {filteredItems.length > 0 ? (
          ...
          <div style={{ ...S.emptyState, marginTop: 32, marginBottom: 32 } as React.CSSProperties}>
            ...
            <Link to="/add-item" style={{ display: 'inline-block', ...S.btnPrimary, textDecoration: 'none' } as React.CSSProperties}>
              Post an Item
            </Link>
          </div>
        )}
